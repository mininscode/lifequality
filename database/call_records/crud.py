...
def get_call_records_by_employee_id(db: Session, employee_id: int, \
                                   skip: int = 0, limit: int = 100): 
    return db.query(models.CallRecord).filter(\
...
def update_call_record_file_path(db: Session, record: schemas.CallRecord, \
                                 file_path: str):
...
def update_call_record_create_date(db: Session, record: schemas.CallRecord, \
                                   created_at: datetime):
...
def update_call_record_citizen_id(db: Session, record: schemas.CallRecord, \
                                  citizen_id: int):
    record.citizen_id = citizen_id
    db.commit()
    db.refresh(record)
    return record

def update_call_record_employee_id(db: Session, record: schemas.CallRecord, \
                                   employee_id: int):
